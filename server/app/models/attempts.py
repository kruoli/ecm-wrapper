...
class ECMAttempt(Base, TimestampMixin):
    ...
    sigma = Column(BigInteger, nullable=True)  # ECM curve parameter
    ...
    def generate_work_hash(cls, composite: str, method: str, b1: int, b2: int = None, 
                          sigma: int = None, curves: int = None) -> str:
        """Generate a hash to detect duplicate work.
        
        Only considers work duplicate if sigma values are explicitly the same.
        Missing sigma values are treated as unique work (different random seeds).
        """
