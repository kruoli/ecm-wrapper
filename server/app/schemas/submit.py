...
class ParametersSchema(BaseModel):
    ...
    sigma: Optional[str] = Field(None, max_length=MAX_ID_LEN, description="ECM curve parameter (can include parametrization like '3:12345' or just '12345')")
...
class FactorWithSigma(BaseModel):
    ...
    sigma: Optional[str] = Field(None, max_length=MAX_ID_LEN, description="Sigma value that found this factor (ECM only)")

class ResultsSchema(BaseModel):
    factor_found: Optional[str] = Field(None, max_length=MAX_NUMBER_LEN, description="Factor found (if any) - DEPRECATED: use factors_found for multiple factors")
...
class SubmitResultRequest(BaseModel):
    ...
    project: Optional[str] = Field(None, max_length=MAX_ID_LEN, description="Project name (optional)")
    ...
    program_version: Optional[str] = Field(None, max_length=MAX_PROGRAM_LEN, description="Program version")
    parameters: ParametersSchema
    results: ResultsSchema
    raw_output: Optional[str] = Field(None, description="Full program output (truncated server-side at MAX_RAW_OUTPUT_LEN)")
    residue_checksum: Optional[str] = Field(None, max_length=MAX_CHECKSUM_LEN, description="SHA-256 checksum of residue file (for stage 2 work from residue pool)")
